...
if ($loadnew) {
	...
	if ($isgroupchat) {
		$stmt = $conn->prepare("SELECT * FROM chatmessages WHERE reciver=? AND isgroupchat=? ORDER BY id DESC LIMIT 15");
		$stmt->bind_param("ss", $otherid, $isgroupchat);
	}
	else{
		$stmt = $conn->prepare("SELECT * FROM chatmessages WHERE ((sender=? AND reciver=?) OR (sender=? AND reciver=?)) AND isgroupchat=? ORDER BY id DESC LIMIT 15");
		$stmt->bind_param("sssss", $thisid, $otherid, $otherid, $thisid, $isgroupchat);
	}
	...
	if ($isgroupchat) {
		...
		$oldestid = $_SESSION['oldestid'];
		$stmt->bind_param("sss", $otherid, $isgroupchat, $oldestid);
	}
	else{
		...
		$oldestid = $_SESSION['oldestid'];
		$stmt->bind_param("ssssss", $thisid, $otherid, $otherid, $thisid, $isgroupchat, $oldestid);
	}
